...
func resourceHDInsightKafkaClusterCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if meta.(*clients.Client).Account.UseMSAL {
		kafkaRestProperty, err = expandKafkaRestProxyProperty(d.Get("rest_proxy").([]interface{}))
	} else {
...
func expandKafkaRestProxyProperty(input []interface{}) (*hdinsight.KafkaRestProperties, error) {
	if len(input) == 0 || input[0] == nil {
		return nil, nil
	}
